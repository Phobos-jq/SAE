...
data_dir = '/data4/jqliu/ML_jq/SAE/activations/ori_feature_layer'   # 训练数据所在目录
model_save_path = '/data4/jqliu/ML_jq/SAE/save/sae_test'       # 保存SAE模型的文件
num_epochs = 256
N_DIRS = 8192
K = 128
...
def sharded_topk(x, k, sh_comm, capacity_factor=None):
    ...
    if capacity_factor is not None:
        k_in = min(int(k * capacity_factor // (sh_comm.size() if sh_comm is not None else 1)), k)
    else:
        k_in = k
...
class FastAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        self.pre_bias = nn.Parameter(torch.zeros(d_model)).to(device)   # 是否需要unsequeeze？
        self.latent_bias = nn.Parameter(torch.zeros(n_dirs_local)).to(device)
    ...
    def forward(self, x):
        class EncWrapper(torch.autograd.Function):
            ...
            def forward(ctx, x, pre_bias, weight, latent_bias):
                # print(f"{x.device=}, {pre_bias.device=}, {weight.device=}, {latent_bias.device=}")
                # print(f"{x.shape}, {pre_bias.shape}, {latent_bias.shape}")
                x = x - pre_bias
            ...
            def backward(ctx, _, grad_vals, __, grad_auxk_vals):
                ...
                return (
                    ...
                    -(grad_sum @ weight),
                    kn.triton_sparse_transpose_dense_matmul(all_inds, all_grad_vals, x, N=self.n_dirs_local),
                    grad_sum,
                )

        pre_bias = self.comms.sh_allreduce_backward(self.pre_bias).to(device)
    ...
    def decode_sparse(self, inds, vals):
        recons = kn.TritonDecoderAutograd.apply(inds, vals, self.decoder.weight)
...
def training_loop_(
    ae, train_acts_iter, num_epochs, loss_fn, lr, comms, eps=6.25e-10, clip_grad=None, ema_multiplier=0.999, logger=None
):
    # print(f"{ae.pre_bias.shape=}")
    if logger is None:
        logger = Logger(dummy=True)

    scaler = torch.amp.GradScaler(device)
    autocast_ctx_manager = torch.amp.autocast(device)
    ...
    for epoch in range(num_epochs):
        train_acts_iter, train_acts_iter_copy = itertools.tee(train_acts_iter)
        for i, flat_acts_train_batch in enumerate(train_acts_iter_copy):
            # print0("1")
            flat_acts_train_batch = flat_acts_train_batch.cuda()
            # print(f"{flat_acts_train_batch.shape=}")
            # print0("2")
            ...
            now = datetime.now()
            print0(f"Time {now}, Epoch {epoch}, Step {global_step}, i {i}, Loss: {loss}")
            ...
            loss = scaler.scale(loss)       # 新的loss乘了一个系数，所以对新loss使用backward会让parameters的grad乘上同一个系数
            loss.backward()
            # print0("3")
            ...
            scaler.unscale_(opt)            # 由于opt管理了parameters，所以这里以opt为参数，实际上是修改parameters的grad

            # gradient clipping
            if clip_grad is not None:
                grad_norm = sharded_grad_norm(ae, comms)
                logger.logkv("grad_norm", grad_norm)
                grads = [x.grad for x in ae.parameters() if x.grad is not None]     # 这里的引用共享了内存，所以直接修改了parameters的grad，opt拥有parameter信息，所以可以直接更新
            ...
            if global_step%50 == 0:
                ...
                print0(f'state_dict saved to {model_save_path} at step {global_step}')
...
def init_from_data_(ae, stats_acts_sample, comms):
    ...
    ae.pre_bias.data = (
        compute_geometric_median(stats_acts_sample[:32768].float().cpu()).median.to(device).float()
    )# .unsqueeze(0)
    ...
    d_model = ae.d_model
    with torch.no_grad():
        x = torch.randn(256, d_model).to(device).to(stats_acts_sample.dtype)
...
class Config:
    ...
    wandb_project: str = "SAE_test"
    wandb_name: str = f"n_dirs={N_DIRS}, k={K}, shard_num=8, batch_num=8, batch_step=8, max_epoch={num_epochs}"


import json

def load_activations(shard_num, batch_num, batch_step):
    for shard_id in range(shard_num):
        for bt in range(batch_num):
            with open(data_dir + f'/__len_64__batch_{(bt+1)*batch_step-1}__shard_{shard_id}.json', 'r') as f:
                activations = json.load(f)
            activations_tensor = torch.tensor([item for sublist in activations for item in sublist])
            # TODO
            # 现在 activations_tensor 的形状是 (num_batches * batch_size * sequence_len, 768)
            yield activations_tensor
...
def main():
    ...
    # with open('/data4/jqliu/ML_jq/SAE/activations/ori_feature_layer/__len_64__shard_0.json', 'r') as f:
    #     activations = json.load(f)
    ...
    # acts_iter = activations_tensor
    # stats_acts_sample = activations_tensor[:32768]

    # acts_iter = acts_iter.unsqueeze(0)
    # # print(f"{acts_iter.shape}")
    ...
    training_loop_(
        ...
        lambda ae, flat_acts_train_batch, recons, info, logger: (
            # MSE
            logger.logkv("train_recons", mse_scale * kn.mse(recons, flat_acts_train_batch))
            # AuxK
            + logger.logkv(
                "train_maxk_recons",
                cfg.auxk_coef
